...
public class ShelfDisplayController extends BaseDisplayController implements Initializable {
    ...
    public void onAdd(ActionEvent event) {
		NavigationState state = new NavigationState<>("Add Shelf", PageType.ADD, "view/page/shelf/ShelfForm.fxml");
    ...
    public <T> void setData(String multiplicity, Map<String, AttributeInfo> scope, T data) {
        ...
        if (multiplicity.endsWith("*")) {
            ShelfDisplayManyController controller = super.loadFXML("view/page/shelf/ShelfDisplayMany.fxml").getController();
            controller.setData(scope, (Supplier<List<TOShelf>>) data);
            return;
        }
        if (multiplicity.endsWith("1")) {
            ShelfDisplayOneController controller = super.loadFXML("view/page/shelf/ShelfDisplayOne.fxml").getController();
