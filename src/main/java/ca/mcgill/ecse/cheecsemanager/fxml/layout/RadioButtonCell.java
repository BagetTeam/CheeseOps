...
public class RadioButtonCell<S> extends TableCell<S, RadioButton> {
  ...
  public RadioButtonCell(ToggleGroup group, Consumer<S> action,
      BiPredicate<S, S> preselectionCondition, S preselectionValue) {
    this.button = new RadioButton();
    button.setToggleGroup(group);
    this.action = action;
  ...
  protected void updateItem(RadioButton radioButton, boolean empty) {
    ...
    if (empty) {
      ...
      // Handle Preselection
      boolean notNull = getTableRow().getItem() != null && preselectionValue != null
          && preselectionCondition != null;
      boolean isPreselected =
          notNull && preselectionCondition.test(getTableRow().getItem(), preselectionValue);
      if (isPreselected) {
