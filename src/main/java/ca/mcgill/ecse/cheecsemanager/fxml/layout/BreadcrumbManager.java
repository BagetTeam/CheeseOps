...
public class BreadcrumbManager {
  ...
  public BreadcrumbManager(BreadCrumbBar<String> breadCrumbBar, Pane parentContainer) {
    ...
    this.breadCrumbBar.setOnCrumbAction(event -> {
      ...
      int rawSelectionDepth =
          depth + 1; // Handle the new item that will be added as part of this event trigger
    ...
    this.breadCrumbBar.setCrumbFactory(crumb -> {
      BreadCrumbBar.BreadCrumbButton breadCrumbButton =
          new BreadCrumbBar.BreadCrumbButton(crumb.getValue());
      breadCrumbButton.setDisable(crumb.getValue().equals("..."));
      return breadCrumbButton;
    });
    this.breadCrumbBar.widthProperty().addListener(
        (obs, oldVal, newVal) -> collapseItems(newVal.doubleValue()));
    this.parentContainer.widthProperty().addListener(
        (obs, oldVal, newVal) -> expandItems(newVal.doubleValue()));
  }
  ...
  private void collapseItems(double breadcrumbWidth) {
    ...
    if (!isOverflowing)
      return;
    if (getCrumbDepth(this.breadCrumbBar.getSelectedCrumb()) < 3)
      return;
    ...
    // If ellipse already there then remove ellipse and update the second item to non ellipse second
    // item
  ...
  private void expandItems(double newWidth) {
    if (Math.abs(newWidth - lastChangedWidth) < THRESHOLD_WIDTH)
      return;
  ...
  /**
   * When a breadcrumb item is clicked, then the number of items after selected item are collapsed
   * (Already done before calling this method) At the same time, same number of items  (if any)
   * before selected item is expanded
   *
  ...
  private void expandIfRequired(int clickedDepth, int totalNavigationDepth) {
    ...
    for (int i = 0; i < finalItems; i++) {
      insertNewItemBefore(secondItem.getChildren().get(0), // get(0) → get(0)
          collapsedStack.remove(collapsedStack.size() - 1)
              .getTitle() // removeLast() → remove(size-1)
      );
    }
    forceUpdateUi(last);
  }
