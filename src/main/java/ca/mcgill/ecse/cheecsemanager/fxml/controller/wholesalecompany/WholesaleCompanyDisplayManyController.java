...
public class WholesaleCompanyDisplayManyController implements Initializable  {
    ...
    public void setData(Map<String, AttributeInfo> scope, Supplier<List<TOWholesaleCompany>> supplier) {
    ...
    public void populateData() {
        ObservableList<TOWholesaleCompany> list = FXCollections.observableArrayList(wholesaleCompanyList);
        
        table.getColumns().clear();
        // Column - name
        if (scope.containsKey("name") && scope.get("name").getScope().equals("BASIC")) {
            TableColumn<TOWholesaleCompany, String> columnName = TableColumnFactory.createTableColumn("name");
            columnName.setCellValueFactory(new PropertyValueFactory<>("name"));
		    table.getColumns().add(columnName);
        }
        // Column - address
        if (scope.containsKey("address") && scope.get("address").getScope().equals("BASIC")) {
            TableColumn<TOWholesaleCompany, String> columnAddress = TableColumnFactory.createTableColumn("address");
            columnAddress.setCellValueFactory(new PropertyValueFactory<>("address"));
		    table.getColumns().add(columnAddress);
        }
        // Column - address
        if (scope.containsKey("monthsAgeds") && scope.get("monthsAgeds").getScope().equals("ID")) {
            Function<TOWholesaleCompany, String> monthsAgedsIdExtractor = e ->
                    FormHelper.isEmpty(e.getMonthsAgeds())
                            ? "-"
                            : Arrays.stream(e.getMonthsAgeds())
                            .map(String::valueOf)
                            .collect(Collectors.joining(", "));
            TableColumn<TOWholesaleCompany, TOWholesaleCompany> columnMonthsAged = TableColumnFactory.createIdColumn("monthsAgeds", monthsAgedsIdExtractor, null);
	...
	protected void updateWholesaleCompany(TOWholesaleCompany wholesaleCompany) {
        NavigationState<TOForm<?, TOWholesaleCompany>> state = new NavigationState<>("Update WholesaleCompany", PageType.UPDATE, "view/page/wholesalecompany/WholesaleCompanyForm.fxml");
    ...
    private void deleteWholesaleCompany(TOWholesaleCompany wholesaleCompany) {
        ...
        ToastFactory.createError(parentContainer, "Deleted WholesaleCompany successfully");
        System.out.println("Deleted WholesaleCompany Successfully");
    }
