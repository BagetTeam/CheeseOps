...
public class CheECSEManager {
  ...
  public boolean setManager(FacilityManager aNewManager) {
    ...
    manager = aNewManager;
    CheECSEManager anOldCheECSEManager =
        aNewManager != null ? aNewManager.getCheECSEManager() : null;
  ...
  public boolean addFarmer(Farmer aFarmer) {
    ...
    boolean isNewCheECSEManager =
        existingCheECSEManager != null && !this.equals(existingCheECSEManager);
  ...
  public boolean addShelveAt(Shelf aShelve, int index) {
    ...
    if (addShelve(aShelve)) {
      ...
      if (index > numberOfShelves()) {
        index = numberOfShelves() - 1;
      }
      shelves.remove(aShelve);
  ...
  /* Code from template association_AddManyToOne */
  public CheeseWheel addCheeseWheel(
      CheeseWheel.MaturationPeriod aMonthsAged, boolean aIsSpoiled, Purchase aPurchase) {
  ...
  public void reinitialize() {
    ...
    // Since all shelves will have a list of shelf locations, need to get all of them from all
    // shelves
    ...
    // Since User is the parent class of Manager and Farmer, we need to get a list of both manager
    // and farmers.
    List<User> allUsers = new ArrayList<>();
    if (getManager() != null)
      allUsers.add(getManager()); // FacilityManager
