...
public class Robot {
  ...
  public Robot(int aDirection, CheeseManager aCheeseManager,
               ShelfSlot aShelfSlot) {
    direction = aDirection;
    if (aCheeseManager == null || aCheeseManager.getRobot() != null) {
      throw new RuntimeException(
          "Unable to create Robot due to aCheeseManager. See "
          + "https://"
    ...
    if (!setShelfSlot(aShelfSlot)) {
      throw new RuntimeException(
          "Unable to create Robot due to aShelfSlot. See "
          + "https://"
          + "manual.umple.org?RE002ViolationofAssociationMultiplicity.html");
    }
  ...
  public CheeseManager getCheeseManager() { return cheeseManager; }
  /* Code from template association_GetOne */
  public ShelfSlot getShelfSlot() { return shelfSlot; }
  ...
  public List<RobotActionLog> getRobotActionLogs() {
    List<RobotActionLog> newRobotActionLogs =
        Collections.unmodifiableList(robotActionLogs);
  ...
  public static int minimumNumberOfRobotActionLogs() { return 0; }
  /* Code from template association_AddManyToOne */
  public RobotActionLog addRobotActionLog(String aSeverity, String aTitle,
                                          String aDetails) {
  ...
  public boolean addRobotActionLogAt(RobotActionLog aRobotActionLog,
                                     int index) {
    boolean wasAdded = false;
    if (addRobotActionLog(aRobotActionLog)) {
      ...
      if (index > numberOfRobotActionLogs()) {
        index = numberOfRobotActionLogs() - 1;
      }
      robotActionLogs.remove(aRobotActionLog);
  ...
  public boolean addOrMoveRobotActionLogAt(RobotActionLog aRobotActionLog,
                                           int index) {
    boolean wasAdded = false;
    if (robotActionLogs.contains(aRobotActionLog)) {
      if (index < 0) {
  ...
  public void delete() {
    ...
    while (robotActionLogs.size() > 0) {
      RobotActionLog aRobotActionLog =
          robotActionLogs.get(robotActionLogs.size() - 1);
  ...
  public String toString() {
    return super.toString() + "["
        ...
        + "cheeseManager = " +
        (getCheeseManager() != null
             ? Integer.toHexString(System.identityHashCode(getCheeseManager()))
             : "null") +
        System.getProperties().getProperty("line.separator") + "  "
        + "shelfSlot = " +
        (getShelfSlot() != null
             ? Integer.toHexString(System.identityHashCode(getShelfSlot()))
             : "null");
  }
}
